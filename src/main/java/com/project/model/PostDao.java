...
public class PostDao {
	...
	public List<PostDO> hotPost(){
		this.sql = "select post_id, position, title, to_char(created_date, 'YYYY-MM-DD HH24:MI:SS') created_date, user_id, content, tags, view_count from (select post_id, position, title, created_date, user_id, content, tags, view_count, row_number() over (order by view_count desc) as row_num from post) where row_num < 6";
		return this.jdbcTemplate.query(sql, new PostRowMapper());
	}
